...
| **Defer textured UV / 1/w plane setup** in `RasterScreenTriTile` until first depth-passing block/pixel | **Loss** on `cpu_mesh_bench textured` (~**−2.7%** interleaved 8 pairs vs eager setup on this VM). Skips `diw/duw/dvw` constants on fully rejected tile visits, but dense textured grids pay extra `EvalAt(w0,w1,w2)` on every SIMD block with `passed!=0` instead of incremental row state. **Do not retry**. |
