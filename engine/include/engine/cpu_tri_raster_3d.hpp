...
/**
 * Screen-space triangle after project (pixel xy, z/w, 1/w) with flat packed color.
 */
struct ScreenTri {
	float x0 = 0.0f;
	float y0 = 0.0f;
...
inline void RasterScreenTriTile(
	...
	const std::uint32_t packed = tri.color;
	const std::uint32_t sa = packed >> 24U;
	const bool opaque = sa == 255U;
	const bool write_depth = opaque; // translucent: test optional; no depth write
...
/**
 * Project a clip-space polygon fan to screen tris.
 */
...
inline void EmitWorldTri(
	...
	const std::uint32_t color,
	const bool cull_backfaces) {
	const ClipVert a = MulViewProj(view_proj, x0, y0, z0);
	const ClipVert b = MulViewProj(view_proj, x1, y1, z1);
	const ClipVert c = MulViewProj(view_proj, x2, y2, z2);
	ClipVert clipped[16];
	const int n = ClipTriangleHomogeneous(a, b, c, clipped);
	ProjectFanToScreen(out, clipped, n, width, height, color, cull_backfaces);
}
